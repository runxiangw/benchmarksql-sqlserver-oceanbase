...
create table bmsql_customer (
  ...
  -- key bmsql_customer_idx1 (c_w_id, c_d_id, c_last, c_first)  -- sql server do not support `key` keyword to create index
);
...
create table bmsql_oorder (
  ...
  -- constraint bmsql_oorder_idx1 unique key (o_w_id, o_d_id, o_c_id, o_id) -- not supported by sqlserver
);

-- Create a unique constraint for bmsql (execute separately)
CREATE UNIQUE INDEX bmsql_oorder_idx1 ON bmsql_oorder(o_w_id, o_d_id, o_c_id, o_id);
